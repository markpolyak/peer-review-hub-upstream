...
4. Бот оставит ссылку на гайдлайн рецензирования и назначит рецензента
5. Проверить 2 чужие работы (откликнуться на назначение в PR)

## Гайдлайны рецензирования

Для каждого ДЗ в `docs/guidelines/hwN.md` хранятся критерии оценки и чеклист рецензента. При открытии PR бот автоматически оставляет ссылку на нужный файл. Если файла для конкретного ДЗ нет — комментарий не отправляется, рецензирование продолжается в обычном режиме.

Чтобы добавить гайдлайн для нового ДЗ — создайте `docs/guidelines/hwN.md` и закоммитьте в `main` до или в момент старта ДЗ.
