...
│   ├── docs/
│   │   └── guidelines/
│   │       ├── hw1.md            # критерии рецензирования для ДЗ 1
│   │       └── hw2.md            # критерии рецензирования для ДЗ 2
│   ├── state/
...
3. assign_reviewer.yml запускается при opened и synchronize:
   → на opened: постит ссылку на гайдлайн рецензирования (docs/guidelines/hwN.md), затем валидирует имя ветки, путь к файлу, логин автора; мягко проверяет заголовок PR
...
├── docs/
│   ├── guidelines/
│   │   ├── hw1.md                    # критерии рецензирования ДЗ 1
│   │   └── hw2.md                    # критерии рецензирования ДЗ 2
│   ├── INSTRUCTIONS.md
...
**Порядок действий при открытии PR:**
1. Бот постит ссылку на `docs/guidelines/hwN.md` (если файл существует) — **до валидации**, чтобы студент видел гайдлайн даже при ошибке в PR.

**Жёсткие проверки** (при нарушении workflow останавливается, рецензент не назначается):
1. Имя ветки начинается с реального логина (`github.actor`) — сравнение без учёта регистра
2. HW-часть ветки строго соответствует формату `hw<число>` (защита от обхода через `hw2-v2`, `hw2-final`)
3. Файл лежит в `reviews/{github.actor}/` — сравнение без учёта регистра

Сравнения регистронезависимы, потому что GitHub-логины case-insensitive: студент может написать имя ветки или путь к файлу строчными буквами — это считается корректным.
...
### Гайдлайны рецензирования

Файлы `docs/guidelines/hwN.md` содержат критерии оценки и чеклист рецензента для каждого ДЗ. Бот автоматически отправляет ссылку на нужный файл при открытии PR. Если файла для ДЗ нет — шаг тихо пропускается.

Для добавления гайдлайна к новому ДЗ создайте `docs/guidelines/hwN.md` и закоммитьте в `main` до или в момент старта ДЗ. За основу можно взять `docs/guidelines/hw1.md`.
